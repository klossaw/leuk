...
itd_data2 <- readr::read_rds("/cluster/home/yjliu_jh/projects/leukemia/data/itd_data2.rds")
...
se <- rds[, cld$disease_type %in% c("AML")]  ## change here if needed
metadata <- colData(se)
...
mut2_list <- list(idh1_mut, idh2_mut)
gene2 <- c("IDH1", "IDH2")
...
meta_plot4x <- left_join(meta_plot4x, aml_anno)  ## switch to other subtype if changes were made above (line:35)
...
ha_x <- HeatmapAnnotation(
  ...
  show_legend = c(TRUE, TRUE, FALSE, rep(F, length(mut_genes) + length(itd_genes) + 1 + length(fus_genes)),
                  T, rep(T, length(gene2)) #rep(F, length(asxl1_hs))),
                  ),
  col = c(list(age = col_fun, 
               gender = setNames(anno_def[1:2], c("female", "male"))),
          auto_anno(meta_plot5, c(mut_genes, itd_genes, itd2_genes, fus_genes#, asxl1_hs
                                  ), c("magenta", "ivory")),
...
quick_heatmap(dat, ha = ha_x, width = 28, height = 16, outdir = "leu/aml/new3", column_split = 26, top_var_percent = 0.95)
...
# ball code
quick_heatmap(dat, ha = ha, outdir = "BALL/all_MEF2D/230710", column_split = 20,
              top_var_percent = 0.95, height = 15, width = 20)
